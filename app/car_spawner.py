...
def DetectClosestTrack(nodes_track_data, local_pos):
	...
	for nodes_data in nodes_track_data:
		...
		for track in nodes_data['track_data']:
			for vector in track['pos']:
				list_x.append(vector.x)
				list_y.append(vector.y)
				list_z.append(vector.z)
...
def HandleFakeCars(scene, res, nodes_track_data, local_pos, spawned_cars, dt, physics):
	...
	for car in spawned_cars:
		...
		if car['turn_index'] < max_index:
			car['vector'] = hg.Lerp(car['track']['pos'][car['turn_index']], car['track']['pos'][car['turn_index'] + 1], car['lerp_value'])
